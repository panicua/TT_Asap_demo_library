...
class BaseBookAPITests(APITestCase):
    def setUp(self):
        ...
        self.detail_url = lambda id_: reverse(
            "books_app:book-detail", args=[id_]
        )
...
class AdminUserBookAPITests(BaseBookAPITests):
    def setUp(self):
        ...
        token = self.client.post(
            ...
            data={"username": self.user.username, "password": "adminpassword"},
        ).data["access"]
...
class DefaultUserBookAPITests(BaseBookAPITests):
    def setUp(self):
        ...
        self.user = User.objects.create_user(
            username="user@test.com", password="userpassword"
        )
        response = self.client.post(
            reverse("token_obtain_pair"),
            data={"username": self.user.username, "password": "userpassword"},
        )
    ...
    def test_create_book(self):
        ...
        response = self.client.post(
            self.list_url, {"title": "The Master and Margarita"}
        )
